...
const AddProductPage = () => {
  ...
  return <div className="min-h-full bg-[#faf8f3] px-4 py-3"><div className="mx-auto max-w-md"><Card className="overflow-hidden rounded-[26px] border-[#e7e4dd]"><CardHeader className="space-y-1 bg-[#f4eadb]"><CardTitle className="text-2xl font-extrabold">Add Product</CardTitle><CardDescription>Enter product details and upload an image.</CardDescription></CardHeader><CardContent className="pt-6"><form onSubmit={submit} className="space-y-4">{error && <p className="text-center text-sm text-red-500">{error}</p>}<div className="space-y-2"><Label>Product Name</Label><input name="name" value={formData.name} onChange={update} onBlur={blur} className="mt-1 block w-full rounded-xl border border-[#e7e4dd] p-2" /><FieldError message={message('name')} /></div><div className="space-y-2"><Label>Price</Label><input type="number" min="0" step="0.01" name="price" value={formData.price} onChange={update} onBlur={blur} className="mt-1 block w-full rounded-xl border border-[#e7e4dd] p-2" /><FieldError message={message('price')} /></div><div className="space-y-2"><Label>Category</Label><input name="category" value={formData.category} onChange={update} onBlur={blur} className="mt-1 block w-full rounded-xl border border-[#e7e4dd] p-2" /><FieldError message={message('category')} /></div><div className="space-y-2"><Label>Description <span className="text-gray-500">(optional)</span></Label><textarea name="description" value={formData.description} onChange={update} className="mt-1 block w-full rounded-xl border border-[#e7e4dd] p-2" /></div><div className="space-y-2"><Label>Image</Label><input type="file" name="image" accept="image/*" ref={fileInputRef} onChange={update} onBlur={blur} className="mt-1 block w-full rounded-xl border border-[#e7e4dd] p-2" /><FieldError message={message('image')} /></div><Button type="submit" className="w-full rounded-xl" disabled={loading}>{loading ? 'Adding Product...' : 'Add Product'}</Button></form></CardContent><CardFooter className="text-center text-sm text-gray-500"><p>Product name, price, category, and image are required.</p></CardFooter></Card></div></div>
}
